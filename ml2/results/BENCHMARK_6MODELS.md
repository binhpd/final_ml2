# BÁO CÁO ĐÁNH GIÁ & SO SÁNH BENCHMARK 6 MÔ HÌNH PHÂN ĐOẠN TÀI LIỆU
**Môn học: Học máy nâng cao (Advanced Machine Learning) — Báo cáo Bài tập lớn**

Báo cáo này trình bày các đánh giá định lượng (Quantitative Analysis) và định tính (Qualitative Analysis) cho hệ thống phân đoạn tài liệu di động. Chúng tôi thực hiện so sánh 6 phiên bản mô hình khác nhau từ baseline đến các mô hình tự huấn luyện và tinh chỉnh sâu (Tuning) trên cùng một cấu hình phần cứng thử nghiệm.

---

## 1. Danh sách 6 Mô hình Đánh giá

Để đánh giá sự tiến triển của hệ thống, 6 mô hình với các đặc điểm kiến trúc và tập dữ liệu huấn luyện khác nhau được đưa vào so sánh:
...
| **`yolo_tuned`** | `exported_models/yolo11n_seg_spine_exclusion_best.pt` | YOLOv11n-seg **tinh chỉnh (Tuning)** tách 2 trang trái/phải, bỏ gáy sách (V1 - 2-Class) | 6.0 MB |
| **`yolo_tuning_v2_full`** | `exported_models/yolo_tuning_v2_full.pt` | YOLOv11n-seg **tinh chỉnh (Tuning)** 1-Class Full Dataset để loại bỏ gáy sách (V2) | 6.0 MB |
...
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| `yolo_old` (COCO) | 2.87M | 0.3351 | 0.3765 | 0.5639 | 0.0146 | **9.2 ms** | **19.6 ms** |
| `u2net_old` (Salient gốc) | 44.01M | 0.9115 | 0.9399 | 0.0333 | **0.3088** | 21.0 ms | 143.5 ms |
| `u2net_trained` (Tự train) | **1.19M** | **0.9732** | **0.9864** | **0.0070** | 0.1259 | 16.1 ms | 89.6 ms |
| `yolo_trained` (Tự train) | 2.83M | **0.9445** | **0.9714** | **0.0114** | 0.0979 | **9.2 ms** | 21.5 ms |
| `yolo_tuned` (Tuning V1 - 2-Class)| 2.83M | 0.3353 | 0.3965 | 0.5536 | 0.0259 | 9.6 ms | 20.3 ms |
| `yolo_tuning_v2_full` (Tuning V2 Full)| 2.83M | **0.9430** | **0.9706** | **0.0120** | **0.0947** | 9.3 ms | 21.1 ms |
...
> Tuy nhiên, với chiến lược **Data Blending 1-Class** của **`yolo_tuning_v2_full`**, mô hình đã lấy lại phong độ xuất sắc với 94.30% mIoU trên ảnh phẳng, trong khi vẫn giữ nguyên siêu năng lực bóc tách gáy sách. Lỗi Catastrophic Forgetting đã bị loại bỏ thành công!

---

## 4. Đánh giá Định lượng Tác vụ Loại bỏ Gáy sách (Spine Exclusion)

Để đánh giá chính xác năng lực loại bỏ gáy sách, chúng tôi tiến hành kiểm định trên tập dữ liệu chuyên biệt gồm các trang sách mở có gáy sách ở giữa (17 ảnh test có gán nhãn đa lớp). 

Kết quả đánh giá định lượng cho từng lớp và trung bình giữa bản Tuning V1 và Tuning V2 Full:

| Mô hình | Lớp Phân Loại | mIoU ↑ | Dice / F1 ↑ | MAE ↓ | Boundary-F1 ↑ | Latency (MPS) ↓ |
|---|---|:---:|:---:|:---:|:---:|:---:|
| **`yolo_tuned` (Tuning V1 - 2-Class)** | left_page | 0.9462 | 0.9720 | 0.0034 | 0.0839 | — |
| | right_page | 0.9730 | 0.9863 | 0.0068 | 0.0617 | — |
| | **average** | **0.9596** | **0.9791** | **0.0051** | **0.0728** | **15.26 ms** |
| **`yolo_tuning_v2_full` (Tuning V2 Full)** | left_page | 0.8925 | 0.9162 | 0.0293 | 0.0595 | — |
| | right_page | 0.9151 | 0.9279 | 0.0304 | 0.0596 | — |
| | **average** | **0.9038** | **0.9221** | **0.0299** | **0.0595** | **34.57 ms** |
...
> Phiên bản `yolo_tuning_v2_full` (áp dụng chiến lược Data Blending 1-Class để tránh quên giấy phẳng) tuy có sự suy giảm nhẹ trên tác vụ gáy sách (mIoU đạt **0.9038**) nhưng đổi lại hiệu năng trên giấy phẳng tăng vọt từ 33.5% lên **94.30%**. Đây là một sự đánh đổi kiến trúc cực kỳ xứng đáng cho một mô hình tổng quát.
...
3. **YOLOv11n-seg — Ứng cử viên số một cho Triển khai On-Device (Mobile):**
   - YOLOv11-seg (`yolo_trained` / `yolo_tuning_v2_full`) cho tốc độ xử lý siêu tốc (**~9.3 ms** trên GPU MPS và chỉ **21.1 ms** trên CPU), nhanh gấp khoảng 4 lần so với U²-Netp và nhanh hơn 6 lần so với U²-Net Full trên CPU.
   - Với khả năng đạt realtime (gần 50 FPS trên CPU mobile), YOLO là lựa chọn hoàn hảo nhất cho Edge AI. Mức độ chính xác (94.30% mIoU) là quá đủ để cắt được một trang giấy trọn vẹn.
   - Quan trọng nhất: YOLO hỗ trợ **nhận diện Instance (từng đối tượng riêng biệt)**, là tiền đề để triển khai bài toán tách trang sách trái/phải (`yolo_tuning_v2_full`) mà mạng phân đoạn ngữ nghĩa cấp pixel (U2-Net) không làm được một cách trực tiếp.

4. **Tính hiệu quả của việc Tuning loại bỏ gáy sách (`yolo_tuning_v2_full`):**
   - Đạt mIoU **0.9038** trên tập test gáy sách chuyên biệt.
