...
def process_dataset(src_img_dir, src_lbl_dir, dst_img_dir, dst_lbl_dir, prefix, max_samples):
    ...
    # Random sub-sampling if limit is set
...
def main():
    ...
    base_dir = Path("datasets") # Changed to use generic datasets folder instead of hardcoded path
    
    # If the workspace path is different
    if not base_dir.exists() and Path("ml2/data").exists():
        base_dir = Path("ml2/data")
        
    # Let's use the explicit absolute path since we know it
    base_dir = Path("/Users/ntcnstudio01/Documents/binhpd2/Final/final_ml2/ml2/data")
    tuning_v2_dir = base_dir / "tuning_v2_full"
    ...
    # NO LIMITS for FULL DATASET
    splits = {"train": None, "val": None, "test": None}
    
    print(f"Creating blended FULL dataset at: {tuning_v2_dir}")
    
    for split, limit in splits.items():
        print(f"Processing split: {split} (Limit: {limit})...")
    ...
    print("Data blending FULL complete!")
